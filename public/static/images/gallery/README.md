...
在分类目录里放一个 `schema.json` 声明文件名各段的含义，文件名用下划线分隔：

`planes/schema.json`

```json
{
  "fields": ["航司", "机型", "机场", "注册号", "彩绘"],
  "facets": ["航司", "机型", "机场", "彩绘"]
}
```
...
```
planes/
  United_B757-200_IAD_N225UA.jpg                 → 航司 United，机型 B757-200，机场 IAD，注册号 N225UA
  United_B777-200_IAD_N78017_Star Alliance.jpg   → 多一段彩绘 Star Alliance
  Aer Lingus_A321-200LR_IAD_EI-LRD.jpg           → 段内可以有空格和连字符
```

- **`fields`** 按顺序对应文件名各段。段数少于 fields 没关系，缺的字段就是空
  （所以「彩绘」这种可有可无的放最后）。多出来的段会被忽略并打印警告。
- **`facets`** 决定哪些字段出现在筛选栏。不写这一项就是全部都做筛选项。
  注册号建议**不要**放进 facets——每张照片一个注册号，做筛选项每项只有 1 张，
  只会把侧边栏撑爆。不做筛选项的字段仍会显示在照片说明里。
- 字段名和取值都支持中文。
- 没有 `schema.json` 的目录不解析文件名，照片只归属于所在分类（城市照片通常
  就不需要 schema）。

相册页侧边栏会按字段名分成「航司」「机型」「机场」等区块：分类单选，标签可
多选且取交集（选 United 再选 B757-200 = United 的 757）。计数是「叠加该标签后
还剩几张」，归零的标签自动隐藏，所以不会点出空结果。
